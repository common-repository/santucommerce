...
var santu=function(){
	...
	this.embed_store=function(){
		$	=	jQuery;
		$(document).on('change','.stebdStor .spGrp0 input',function(){
			v	=	$(this).val();
			sc	=	$('.stescat');
			sp	=	$('.stessp');
			if(v==2){
		...
		$(document).on('click','.santuModalBoxE .smbtnOk',function(){
			...
			if(v==2){
				l=$('.stessp').find(':selected').val();
			}else{
				l=$('.stescat').val();
			}
			if(cf_is_valid_URL(l)==true){
				$('.santuModalBoxE').hide();
				tinymce.activeEditor.execCommand('mceInsertContent',false,'[santu_embed url= "'+l+'"]')
			}
...
jQuery(document).ready(function(e){(function($){
	...
	*/
					...
					if(value){
						if(delimiter==':hover'){
							if(property=='border-radius'||property=='text-shadow'||property=='border'){
								pv1=property+':'+value+';';pv1=pv1.replace('!important !important','!important');o+=pv1;
							}
							else{pv1=property+':'+value+' !important;';pv1=pv1.replace('!important !important','!important');o+=pv1;}
						}
						else{
	...
	var santu_change_prview=function(){
		$(document).on('change','.stSpecial_link ',function(){v=$(this).find(':selected').attr('dval');santu_special_link_change_txt(v);cf_setCookie('stBtnTxt',v)});
		$(document).on('change','#button_special_links',function(){if($(this).is(':checked')){v=$('#special_link_button').find(':selected').attr('dval');santu_special_link_change_txt(v)}});
		$(document).on('click','.btnColorRadio input[name="btnColor"]',function(){v=$(this).val();tg=$('.santuModalBoxB .smPreview a');if(v==2){tg.attr('id','stPrwBtnHover')}else{tg.attr('id','');tg.addClass('stPrwBtn')}});
		$(document).on('change','.stInsertBuyNowButton input[name="source_type"]',function(){v=$(this).val();if(v=='address'){t=$('.stBtnSubTxt').val();santu_special_link_change_txt(t)}})
	}
	var santu_special_link_change_txt=function(v){
		...
		if($('.stPrwBtn').find("img").length>0){
			...
			$('.stPrwBtn').html(' ');
			if(smIcPo=='right'){$('.stPrwBtn').html(v+'<img src="'+src+'" width="'+size+'" height="'+size+'" style="margin-left:'+smIcPd+'">')}
			else{$('.stPrwBtn').html('<img src="'+src+'" width="'+size+'" height="'+size+'" style="margin-right:'+smIcPd+'">'+v)}
		}
	...
	var santu_set_editor_value = function(k,v,h='',arr=''){
		...
		fc=$('#smBtnFontColor');gt=$('#smBtnGradientTopColor');
		gb=$('#smBtnGradientBottomColor');bc=$('#smBtnBorderColor');
		fch=$('#smBtnFontColorH');gth=$('#smBtnGradientTopColorH');
		gbh=$('#smBtnGradientBottomColorH');bch=$('#smBtnBorderColorH');
		if(h==''){
	...
	santu_save_custom_button=function(url,title){
		...
		if(normal!=false){ s1	=	normal}else{ s1 = ''; }
		hover	=	cf_getCookie('stBtnhover');
		if(hover!=false){s2	=	hover;}
		else{ s2	=	''; }
		active	=	''; s3='';
		after		=	''; s4='';
		before	=	''; s5='';
			...
			success:function(data){
				...
				if(data){
					...
					if(cll=='collection'){
						...
						if(cll){
							santu_get_collection_button(nbid,cll);
						}
					}
					else{
	...
	santu_set_title=function(t){
		if(t){
			$('.stMButton_text,.stBtnSubTxt').val(t);
